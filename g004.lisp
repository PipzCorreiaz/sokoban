      ...
      (let ((caixa (nth i (first estado)))
        ...
        (dolist (jogada (jogadas-validas2 (mapa-sokoban-mapa *mapa*) ocupadas (first caixa) (second caixa)))
          ...
          (unless (or (casa-preenchida ocupadas (first proxima-posicao-homem) (second proxima-posicao-homem)) (casa-ocupada *mapa* (first proxima-posicao-homem) (second proxima-posicao-homem)))
            (format t "PROXIMA: ~A~%" proxima-posicao-homem)
            (setf (nth i (first novo-estado)) jogada)
            (if (not (null homem))
                (setf caminho (reverse (encontra-caminho *mapa* (first estado) (first homem) (second homem) (first proxima-posicao-homem) (second proxima-posicao-homem))))
                (setf caminho (list proxima-posicao-homem jogada)))
            (unless (null caminho)
              (format t "CAMINHO: ~A~%" caminho)
              (setf (second novo-estado) (nconc caminho (cdr (second novo-estado))))
...
(defun novos-passos (caminho caixas homem)
  (let ((novo-caminho nil)
    ...
    (format t "CAMINHO FINAL: ~A~%" novo-caminho)
